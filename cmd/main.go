...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "32cc4a18.example.com",
	...
	if err := controller.NewPostgresClusterReconciler(
		mgr.GetClient(), mgr.GetScheme(), mgr.GetEventRecorder("postgrescluster-operator"),
	).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", controller.PostgresClusterControllerName)
		os.Exit(1)
	}
	if err := controller.NewValkeyClusterReconciler(
		mgr.GetClient(), mgr.GetScheme(), mgr.GetEventRecorder("valkeycluster-operator"),
	).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", controller.ValkeyClusterControllerName)
		os.Exit(1)
	}
	if err := controller.NewGrafanaInstanceReconciler(
		mgr.GetClient(), mgr.GetScheme(), mgr.GetEventRecorder("grafanainstance-operator"),
	).SetupWithManager(mgr); err != nil {
	...
	if err := controller.NewKafkaClusterReconciler(
		mgr.GetClient(), mgr.GetScheme(), mgr.GetEventRecorder("kafkacluster-operator"),
	).SetupWithManager(mgr); err != nil {
